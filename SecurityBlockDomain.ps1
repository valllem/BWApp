...
write-host -ForegroundColor Yellow 'Block Domain (SPAM)'
...
Set-HostedContentFilterPolicy "BusinessWorks Policy" -BlockedSenderDomains @{add="$x"} 
Add-Content "$logfile" "$DateTime"
Add-Content "$Logfile" "Added domain to the Businessworks Policy"
}
catch  {
Write-Host -ForegroundColor Cyan "Blocking " $x
Set-HostedContentFilterPolicy default -BlockedSenderDomains @{add="$x"}
Add-Content "$logfile" "$DateTime"
Add-Content "$Logfile" "Added domain to the Default Policy"
}
